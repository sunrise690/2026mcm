...
class Q3ParticleController(_base.Q3ParticleController):
    ...
    def run(self):
        ...
        try:
            ...
            for step in range(self.max_steps):
                ...
                if detected:
                    ...
                    if not self.discovery_verified():
                        ...
                        cheap_progress = (cert_gain == unresolved or
                            (cert_gain >= 0.45 * unresolved and
                             cert_gain >= 0.015 * len(self.certificate.centers)))
                ...
                if discovery_done:
                    ...
                    self.completion_reason = ("maximum_16_sources_cleared" if len(self.cleared) >= MAXN
                        else "continuous_domain_certified_and_all_detected_cleared")
                    break
                probable_done = (known >= 10 and len(self.cleared) >= 9 and
                    self.posterior_same_count() >= self.stop_threshold(known))
            ...
            self.completion_certificate.update({"completion_verified": self.completion_verified,
                "completion_reason": self.completion_reason, "cleared_count": len(self.cleared),
                "known_count": self.known(), "supplemental_coverage_probes": self.coverage_probe_count})
