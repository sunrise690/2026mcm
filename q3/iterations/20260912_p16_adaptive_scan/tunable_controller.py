...
class Q3ParticleController:
    ...
    def _maybe_commit_terminal_probe(self,items,order):
        if (not int(self.g("terminal_route_probe")) or self.terminal_probe_used or self.terminal_probe_active
            or self.committed_probe is not None or self.known()>=MAXN
            or len(items)<int(self.g("terminal_probe_min_pending")) or len(items)>int(self.g("terminal_probe_max_pending"))):return False
