...
DEFAULT_TUNE = {
    ...
    "particle_estimator": 0,
    "particle_mode_bin_m": 25.0,
    "particle_mean_weight": 0.25,
    "particle_ls_weight": 0.5,
    "particle_ls_min_cross": 0.22,
    "particle_ls_max_shift_m": 60.0,
    "particle_ls_radius_frac": 0.5,
    "bearing_tol_deg": 1.02,
    "coverage_grid_h": 45.0,
    "best_probe_dist_div": 9000.0,
    "explore_cost_ratio_before10": 0,
    "explore_cost_ratio_min_known": 0,
    "explore_cost_ratio_pd_power": 1.0,
    ...
    "commit_min_known": 10,
    "direct_probe_min_known": 0,
    "direct_probe_max_detour_m": 1000000000.0,
    ...
    "soft_stop_target": -1.0,
    "source_detect_bonus_s": 0.0,
    "source_detect_min_known": 10,
    "source_choice_pool": 5,
    "dynamic_route_enabled": 0,
    "dynamic_route_discovery_value_s": 140.0,
    "dynamic_route_tail_weight": 1.0,
    "dynamic_route_max_p16": 1.01,
    "dynamic_route_max_known": 16,
    "dynamic_route_min_same": 0.0,
    "route_batch_lock_enabled": 0,
    "route_batch_lock_min_items": 3,
    "route_batch_insert_max_detour_m": -1.0,
    "route_hysteresis_enabled": 0,
    "route_hysteresis_extra_m": 100.0,
    "adaptive_route_scan": 0,
    "route_scan_value_s": 900.0,
    "route_scan_lowp16_enabled": 0,
    "route_scan_lowp16_min_known": 10,
    "route_scan_lowp16_cut": 0.15,
    "route_scan_lowp16_value_s": 650.0,
    "route_scan_min_pd": 0.0,
    "route_scan_high_known_floor": 0.0,
    "route_scan_high_known_min_known": 99,
    "route_scan_partial_limit": 0,
    "route_scan_partial_min_known": 0,
    "route_scan_partial_below_pd": 1.01,
    "route_scan_partial_p16_min": -1.0,
    ...
    "service_waypoint_unknown_limit": 0,
    ...
    "bootstrap_angle_n": 24,
    "bootstrap_sparse_radii": [700,900,1100],
    "bootstrap_sparse_max_known": -1,
    "bootstrap_mid_radii": [700,900,1100],
    "bootstrap_mid_min_known": 99,
    "bootstrap_mid_max_known": -1,
    "bootstrap_unknown_scan": 1,
    "bootstrap_unknown_limit": 0,
    "deferred_blind_scan": 0,
    "deferred_blind_batch": 4,
    "deferred_blind_min_pd": 0.10,
    "deferred_blind_max_known": 15,
    "bootstrap_first_source_w": 0.0,
    "route_end_explore": 0,
    "route_end_explore_weight": 1.0,
    "late_route_end_explore": 0,
    "late_route_end_min_known": 10,
    "late_route_end_max_pending": 4,
    "late_route_end_weight": 1.0,
    "boundary16_route_probe": 0,
    "boundary16_min_known": 15,
    ...
    "midroute_discovery_probe": 0,
    "midroute_probe_min_known": 8,
    "midroute_probe_max_known": 9,
    "midroute_probe_min_cleared": 3,
    "midroute_probe_min_pending": 2,
    "preclear_probe_info_limit": 0,
    "frontload_probe_count": 0,
    "frontload_probe_info_limit": 3,
    "channel_posterior_enabled": 0,
    "channel_count_posterior_enabled": 0,
    "channel_explore_scan_limit": 0,
    "channel_candidate_pool": 60,
    "channel_probe_dist_div": 7000.0,
    "channel_scan_early_stop": 1,
    "terminal_route_probe": 0,
    "terminal_probe_min_known": 10,
    "terminal_probe_min_pending": 2,
    "terminal_probe_max_pending": 4,
    "terminal_probe_max_detour_m": 250.0,
    "terminal_probe_min_hppd": 0.05,
    "terminal_probe_min_same": 0.45,
    "terminal_probe_force_p16": 0.12,
    "terminal_probe_empty_accept_ratio": 0.92,
    "terminal_probe_empty_p16_max": 0.18,
    "tail_value_probe": 0,
    "tail_value_min_known": 8,
    "tail_value_max_known": 9,
    "tail_value_min_pending": 2,
    "tail_value_max_pending": 6,
    "tail_value_candidate_pool": 120,
    "tail_value_min_pd": 0.10,
    "tail_value_max_detour_m": 900.0,
    "tail_value_scan_risk_scale": 1.0,
    "tail_value_distance_scale": 1.0,
    "tail_value_net_min_s": 0.0,
}
...
class ParticleBelief:
    ...
    def estimate(self):
        ...
        xy=self.X[:,:2]
        mode=int(self.tune.get("particle_estimator",0))
        if mode==1:
            c=np.mean(xy,axis=0)
        elif mode==4:
            med=np.median(xy,axis=0);dirs=[h for h in self.hist if h[1]=='direction']
            c=med
            if len(dirs)>=2:
                A=[];yy=[];cross=0.0;es=[]
                for p0,_,th in dirs:
                    a=math.radians(float(th));e=np.array([math.cos(a),math.sin(a)]);es.append(e)
                    n=np.array([-e[1],e[0]]);A.append(n);yy.append(float(np.dot(n,p0)))
                for i,e in enumerate(es):
                    for f in es[:i]:cross=max(cross,abs(float(e[0]*f[1]-e[1]*f[0])))
                if cross>=float(self.tune.get("particle_ls_min_cross",0.22)):
                    try:
                        ls=np.linalg.lstsq(np.asarray(A,float),np.asarray(yy,float),rcond=None)[0]
                        nl=float(np.linalg.norm(ls))
                        if nl>REGION:ls*=REGION/nl
                        delta=ls-med;nd=float(np.linalg.norm(delta));rmed=float(np.percentile(np.linalg.norm(xy-med,axis=1),90))
                        cap=min(float(self.tune.get("particle_ls_max_shift_m",60.0)),float(self.tune.get("particle_ls_radius_frac",0.5))*rmed)
                        if nd>cap>0:ls=med+delta*(cap/nd)
                        w=float(np.clip(self.tune.get("particle_ls_weight",0.5),0.0,1.0))
                        c=(1.0-w)*med+w*ls
                    except Exception:pass
        elif mode==3:
            w=float(np.clip(self.tune.get("particle_mean_weight",0.25),0.0,1.0))
            c=(1.0-w)*np.median(xy,axis=0)+w*np.mean(xy,axis=0)
        elif mode==2:
            h=max(5.0,float(self.tune.get("particle_mode_bin_m",25.0)))
            ij=np.floor((xy+REGION)/h).astype(np.int32)
            keys=ij[:,0].astype(np.int64)*10000+ij[:,1].astype(np.int64)
            vals,cnt=np.unique(keys,return_counts=True);key=vals[int(np.argmax(cnt))]
            cell=ij[keys==key];center_idx=int(np.argmax(cnt))
            ii=int(key//10000);jj=int(key%10000)
            near=(np.abs(ij[:,0]-ii)<=1)&(np.abs(ij[:,1]-jj)<=1)
            c=np.median(xy[near],axis=0) if np.any(near) else np.median(xy,axis=0)
        else:
            c=np.median(xy,axis=0)
        r=np.percentile(np.linalg.norm(xy-c,axis=1),90);return c,float(r)
...
class Q3ParticleController:
    def __init__(self,client,tune=None):
        ...
        self.channel_d2={ch:np.full(len(self.cover.p),np.inf,dtype=np.float32) for ch in range(1,21)}
        self._scan_seq=0;self.committed_probe=None;self.sweep_angle_deg=None;self.soft_stop_probes=0;self.empty_probe_streak=0
        self.route_batch_queue=[]
        self.route_preferred_queue=[]
        self.terminal_probe=None;self.terminal_probe_active=False;self.terminal_probe_used=False;self.executing_terminal_probe=False;self.terminal_probe_empty=False
        self.midroute_probe_used=False
        self.deferred_bootstrap_channels=set()
    ...
    def measure(self,p,ch):
        ...
        if was_unknown and r=='no_signal' and int(self.g("channel_posterior_enabled")):
            dd=np.sum((self.cover.p-p)**2,axis=1);self.channel_d2[ch]=np.minimum(self.channel_d2[ch],dd)
    ...
    def scan_unknown(self,p):
        ...
        if gain>0 and self.terminal_probe_active:
            self.terminal_probe=None;self.terminal_probe_active=False
            print("[路线内终检] 后续发现改变后验，取消旧终检点并等待重算",flush=True)
        ...
        return gain
    def scan_unknown_partial(self,p,limit):
        p=np.asarray(p,float);chs=self.unknown();limit=max(0,min(int(limit),len(chs)))
        if limit<=0:return 0
        # Rank channel-specific late-fill value at this location.
        rc=getattr(self.c,'receiver_channel',None)
        if int(self.g("channel_posterior_enabled")):
            score=dict(self.channel_detect_scores(p))
            chs.sort(key=lambda ch:(-score.get(ch,0.0),0 if ch==rc else 1,ch))
        else:
            chs.sort(key=lambda ch:(len(self.B[ch].hist),0 if ch==rc else 1,ch))
        chosen=chs[:limit];before=self.known();self._scan_seq+=1
        print(f"[渐进普查{self._scan_seq}] 到达 {self._pos(p)}，轮换扫描 {len(chosen)}/{len(chs)} 个未知频道...",flush=True)
        for ch in chosen:
            if self.known()>=MAXN:break
            self.measure(p,ch)
            if (int(self.g("channel_posterior_enabled")) and int(self.g("channel_scan_early_stop"))
                and self.known()>=10 and self.posterior_same_count()>=self.stop_threshold()):break
        gain=self.known()-before
        # Partial scans are channel-specific and therefore do not certify global
        # geometric coverage. Dedicated full surveys remain the stopping evidence.
        print(f"[渐进普查{self._scan_seq}] 完成，新发现 {gain} 个 | {self._progress()}",flush=True)
        return gain

    def scan_deferred_blind(self,p,limit):
        """补扫第二固定测站欠测的频道，不增加额外移动。"""
        p=np.asarray(p,float)
        chs=[ch for ch in self.deferred_bootstrap_channels if self.B[ch].status=='unknown']
        if not chs:return 0
        # 优先补扫历史观测最少的频道；同分时优先当前接收频道，减少换频。
        rc=getattr(self.c,'receiver_channel',None)
        chs.sort(key=lambda ch:(len(self.B[ch].hist),0 if ch==rc else 1,ch))
        chosen=chs[:max(1,min(int(limit),len(chs)))]
        before=self.known();self._scan_seq+=1
        print(f"[盲区补扫{self._scan_seq}] 清除路径点{self._pos(p)}，补扫欠测频道 {len(chosen)}/{len(chs)}",flush=True)
        for ch in chosen:
            if self.known()>=MAXN:break
            self.measure(p,ch);self.deferred_bootstrap_channels.discard(ch)
        gain=self.known()-before
        print(f"[盲区补扫{self._scan_seq}] 新发现 {gain} 个，欠测剩余 {len(self.deferred_bootstrap_channels)}",flush=True)
        return gain
    ...
    def bootstrap_point(self):
        ...
        k0=self.known()
        if k0<=int(self.g("bootstrap_sparse_max_known")):radii=self.g("bootstrap_sparse_radii")
        elif int(self.g("bootstrap_mid_min_known"))<=k0<=int(self.g("bootstrap_mid_max_known")):radii=self.g("bootstrap_mid_radii")
        else:radii=self.g("bootstrap_radii")
        for r in radii:
            n=max(8,int(self.g("bootstrap_angle_n")))
            for k in range(n):
    ...
    def posterior_expected_remaining(self):
        k=self.known();po=self.count_posterior()
        return float(sum(max(0,n-k)*w for n,w in po.items()))

    def heuristic_route_length(self,start,items):
        if not items:return 0.0
        pts=np.asarray([p for _,p in items],float);n=len(pts);start=np.asarray(start,float)
        best=float('inf')
        starts=np.argsort(np.linalg.norm(pts-start,axis=1))[:min(4,n)]
        for st in starts:
            unused=set(range(n));unused.remove(int(st));o=[int(st)]
            while unused:
                j=min(unused,key=lambda z:float(np.linalg.norm(pts[z]-pts[o[-1]])))
                o.append(j);unused.remove(j)
            def plen(order):
                v=float(np.linalg.norm(pts[order[0]]-start))
                for a,b in zip(order,order[1:]):v+=float(np.linalg.norm(pts[b]-pts[a]))
                return v
            val=plen(o)
            for _ in range(2):
                changed=False
                for a in range(n-2):
                    for b in range(a+2,n):
                        q=o[:a+1]+list(reversed(o[a+1:b+1]))+o[b+1:];v=plen(q)
                        if v+1e-9<val:o,val,changed=q,v,True
                if not changed:break
            best=min(best,val)
        return best

    def dynamic_source_choice(self,order):
        if not order:return None
        cur=np.asarray(self.c.position,float);pool=order[:max(1,int(self.g("source_choice_pool")))]
        er=max(0.0,self.posterior_expected_remaining());m=len(self.unknown());scan_s=5.0*m+max(0,m-1)
        bonus=float(self.g("dynamic_route_discovery_value_s"));tw=float(self.g("dynamic_route_tail_weight"));best=None
        for ch,p in pool:
            p=np.asarray(p,float);b=self.B[ch];_,rad=b.estimate();q=p
            if rad*float(self.g("service_aware_uncertainty_scale"))>float(self.g("service_uncertainty_trigger")):
                q=self.service_measure_point(b,p,current=cur)
            first=(float(np.linalg.norm(q-cur))+float(np.linalg.norm(p-q)))/5.0
            rest=[x for x in order if x[0]!=ch]
            tail=self.heuristic_route_length(p,rest)/5.0
            pd=self.cover.conditional_detect(p);scan_penalty=scan_s if pd>=0.08 else 0.0
            score=first+tw*tail+scan_penalty-bonus*er*pd
            rec=(score,first+tail,-pd,ch,p)
            if best is None or rec[:3]<best[:3]:best=rec
        return best[3],np.asarray(best[4],float)

    def source_order(self,items,discovery_done=False):
        if (int(self.g("late_route_end_explore")) and not discovery_done
            and self.known()>=int(self.g("late_route_end_min_known"))
            and 2<=len(items)<=int(self.g("late_route_end_max_pending")) and self.known()<MAXN):
            target=self.exploration_point()
            if target is not None:
                return self.exact_order(items,route_target=target,endpoint_weight=float(self.g("late_route_end_weight")))
    ...
    def _joint_next_base(self,items,soft_done=False):
        ...
        # Near the 16-source upper boundary, route the statistically required
        # terminal survey together with pending clears. This avoids clearing one
        # side of the disk and then crossing it only to reveal source 16.
        k=self.known()
        order=self.source_order(items,soft_done)
        if self.terminal_probe_active and self.terminal_probe is not None:
            tp=np.asarray(self.terminal_probe,float)
            if len(items)<=1:
                self.terminal_probe_active=False;self.executing_terminal_probe=True
                return ('probe',None,tp)
            ao=self.exact_order(list(items)+[(-998,tp)])
            if ao:
                ch,p=ao[0]
                if ch==-998:
                    self.terminal_probe_active=False;self.executing_terminal_probe=True
            ...
            self.terminal_probe_active=False;self.terminal_probe=None
        self._maybe_commit_terminal_probe(items,order)
        if self.terminal_probe_active and self.terminal_probe is not None:
            tp=np.asarray(self.terminal_probe,float);ao=self.exact_order(list(items)+[(-998,tp)])
            if ao:
                ch,p=ao[0]
                if ch==-998:
                    self.terminal_probe_active=False;self.executing_terminal_probe=True
                    return ('probe',None,np.asarray(p,float))
                return ('source',ch,np.asarray(p,float))
        if (int(self.g("boundary16_route_probe")) and k>=int(self.g("boundary16_min_known")) and k<MAXN
            and self.committed_probe is None):
            ep=self.exploration_point()
            if ep is not None:self.committed_probe=np.asarray(ep,float).copy()
        ...
        if can_commit:
            ...
            if ao:
                ...
                if ch==-999:
                    ...
                    return ('probe',None,np.asarray(p,float))
                return ('source',ch,np.asarray(p,float))
        if (self.known()>=int(self.g("direct_probe_min_known")) and best[1]==0
            and best[0]<=allowance and best[0]<=float(self.g("direct_probe_max_detour_m"))):
            print(f"[立即探测判据] known={self.known()} pending={len(items)} detour={best[0]:.0f}m Pd={pd:.3f} hp={hp:.3f} p16={self.posterior_p16():.3f}",flush=True)
            return ('probe',None,probe)
        if (int(self.g("dynamic_route_enabled"))
            and self.known()<=int(self.g("dynamic_route_max_known"))
            and self.posterior_p16()<=float(self.g("dynamic_route_max_p16"))
            and self.posterior_same_count()>=float(self.g("dynamic_route_min_same"))):
            chosen=self.dynamic_source_choice(order)
            if chosen is not None:return ('source',chosen[0],chosen[1])
    ...
    def _maybe_commit_terminal_probe(self,items,order):
        tail_enabled=int(self.g("tail_value_probe"))
        regular_enabled=int(self.g("terminal_route_probe"))
        if ((not regular_enabled and not tail_enabled) or self.terminal_probe_used or self.terminal_probe_active
            or self.committed_probe is not None or self.known()>=MAXN
            or not order):return False
        k=self.known();ps=self.posterior_same_count()
        if (tail_enabled and int(self.g("tail_value_min_known"))<=k<=int(self.g("tail_value_max_known"))
            and int(self.g("tail_value_min_pending"))<=len(items)<=int(self.g("tail_value_max_pending"))):
            cur=np.asarray(self.c.position,float);pts=[np.asarray(p,float) for _,p in order]
            hp=max(0.0,min(1.0,1.0-ps));m=len(self.unknown());scan_s=5.0*m+max(0,m-1)
            best=None
            for q in self._explore_candidates(cur,max_pool=int(self.g("tail_value_candidate_pool"))):
                q=np.asarray(q,float);pd=self.cover.conditional_detect(q)
                if pd<float(self.g("tail_value_min_pd")):continue
                detour,pos=self._route_insertion(order,q)
                # The probe is forced before the final clear.  Its measurable
                # value is the cross-field trip avoided when the same posterior
                # hot spot would otherwise be visited from the route endpoint.
                # Only the probability-weighted distance is credited; the scan
                # charge remains explicit, scaled down only in risk variants.
                endpoint_dist=float(np.linalg.norm(q-pts[-1]))
                saved_s=hp*pd*endpoint_dist/5.0*float(self.g("tail_value_distance_scale"))
                cost_s=detour/5.0+float(self.g("tail_value_scan_risk_scale"))*scan_s
                net_s=saved_s-cost_s
                rec=(net_s,pd,endpoint_dist,-detour,q,pos)
                if best is None or rec[:4]>best[:4]:best=rec
            if (best is not None and best[0]>=float(self.g("tail_value_net_min_s"))
                and -best[3]<=float(self.g("tail_value_max_detour_m"))):
                self.terminal_probe=np.asarray(best[4],float).copy();self.terminal_probe_active=True;self.terminal_probe_empty=False
                print(f"[尾段价值探测] 预提交 {self._pos(self.terminal_probe)}，净收益≈{best[0]:.1f}s，绕行≈{-best[3]:.0f}m，端点回访≈{best[2]:.0f}m，P发现≈{best[1]:.2f}",flush=True)
                return True
        if (not regular_enabled or len(items)<int(self.g("terminal_probe_min_pending"))
            or len(items)>int(self.g("terminal_probe_max_pending"))):return False
        if k<int(self.g("terminal_probe_min_known")) or ps<float(self.g("terminal_probe_min_same")):return False
        q=self.exploration_point()
        if q is None:return False
        q=np.asarray(q,float);cur=np.asarray(self.c.position,float);pts=[np.asarray(p,float) for _,p in order]
        # The virtual survey must occur before the final clear, so only interior
        # insertion positions are considered. Its scan cost is unavoidable; the
        # gate controls only extra travel relative to the current clear route.
        best=(float('inf'),0)
        for pos in range(len(pts)):
            a=cur if pos==0 else pts[pos-1];b=pts[pos]
            delta=float(np.linalg.norm(q-a)+np.linalg.norm(b-q)-np.linalg.norm(b-a))
            if delta<best[0]:best=(delta,pos)
        pd=self.cover.conditional_detect(q);hp=max(0.0,min(1.0,1.0-ps));p16=self.posterior_p16()
        value_ok=(hp*pd>=float(self.g("terminal_probe_min_hppd")) or p16>=float(self.g("terminal_probe_force_p16")))
        if best[0]>float(self.g("terminal_probe_max_detour_m")) or not value_ok:return False
        self.terminal_probe=q.copy();self.terminal_probe_active=True;self.terminal_probe_empty=False
        print(f"[路线内终检] 预提交 {self._pos(q)}，增量路程≈{best[0]:.0f}m，P发现≈{pd:.2f}，P16≈{p16:.2f}",flush=True)
        return True
    ...
    def _route_batch_next(self,items):
        detected={ch:np.asarray(p,float) for ch,p in items if ch not in self.cleared}
        self.route_batch_queue=[ch for ch in self.route_batch_queue if ch in detected]
        if not self.route_batch_queue:return None
        cap=float(self.g("route_batch_insert_max_detour_m"))
        if cap>=0:
            pending=[ch for ch in detected if ch not in self.route_batch_queue]
            for ch in pending:
                q=detected[ch];pts=[np.asarray(self.c.position,float)]+[detected[x] for x in self.route_batch_queue]
                best=(float("inf"),len(self.route_batch_queue))
                for i in range(len(self.route_batch_queue)+1):
                    a=pts[i]
                    if i<len(self.route_batch_queue):
                        b=pts[i+1];delta=float(np.linalg.norm(q-a)+np.linalg.norm(b-q)-np.linalg.norm(b-a))
                    else:delta=float(np.linalg.norm(q-a))
                    if delta<best[0]:best=(delta,i)
                if best[0]<=cap:self.route_batch_queue.insert(best[1],ch)
        ch=self.route_batch_queue[0]
        p=self.B[ch].estimate()[0]
        return None if p is None else (ch,np.asarray(p,float))

    def _seed_route_batch(self,items,chosen,soft_done):
        if not int(self.g("route_batch_lock_enabled")) or len(items)<int(self.g("route_batch_lock_min_items")):return
        order=self.source_order(items,soft_done)
        seq=[chosen]+[ch for ch,_ in order if ch!=chosen]
        self.route_batch_queue=[]
        for ch in seq:
            if ch not in self.route_batch_queue:self.route_batch_queue.append(ch)

    def _route_hysteresis_choice(self,items,ans,soft_done):
        if not int(self.g("route_hysteresis_enabled")) or ans[0]!='source':return ans
        detected={ch:np.asarray(p,float) for ch,p in items if ch not in self.cleared}
        self.route_preferred_queue=[ch for ch in self.route_preferred_queue if ch in detected]
        base_ch=ans[1];base_p=np.asarray(ans[2],float);cur=np.asarray(self.c.position,float)
        if self.route_preferred_queue:
            old_ch=self.route_preferred_queue[0]
            if old_ch!=base_ch:
                old_p=self.B[old_ch].estimate()[0]
                if old_p is not None:
                    extra=float(np.linalg.norm(np.asarray(old_p,float)-cur)-np.linalg.norm(base_p-cur))
                    if extra<=float(self.g("route_hysteresis_extra_m")):
                        return ('source',old_ch,np.asarray(old_p,float))
        order=self.source_order(items,soft_done)
        self.route_preferred_queue=[base_ch]+[ch for ch,_ in order if ch!=base_ch]
        return ans

    def joint_next(self,items,soft_done=False):
        if int(self.g("route_batch_lock_enabled")):
            nxt=self._route_batch_next(items)
            if nxt is not None:return ('source',nxt[0],nxt[1])
        mode=self.geometry_mode();k=self.known()
        if mode==2:ans=self._joint_next_information(items,soft_done)
        elif mode==3:ans=self._joint_next_sweep(items,soft_done)
        elif mode==4 and 10<=k<=int(self.g("adaptive_joint_max_known")):
            ans=self._joint_next_information(items,soft_done)
        else:ans=self._joint_next_base(items,soft_done)
        ans=self._route_hysteresis_choice(items,ans,soft_done)
        if ans[0]=='source':self._seed_route_batch(items,ans[1],soft_done)
        return ans
        ...
        for it in range(int(self.g("service_iters"))):
            ...
            if r>float(self.g("service_uncertainty_trigger")) and it==0:
                ...
                if (int(self.g("service_waypoint_unknown_scan")) and self.known()<=int(self.g("service_waypoint_unknown_max_known"))
                    ...
                    lim=int(self.g("service_waypoint_unknown_limit"))
                    if lim>0 and lim<len(self.unknown()):
                        print(f"  [定位兼找补] 交会站轮换补扫 {lim}/{len(self.unknown())} 个未知频道",flush=True)
                        self.scan_unknown_partial(mp,lim)
                    else:
    ...
    def _exploration_point_base(self,target_thr=None):
        ...
        if k<10:
            if (int(self.g("explore_cost_ratio_before10"))
                and k>=int(self.g("explore_cost_ratio_min_known"))):
                cand=self._explore_candidates(cur,max_pool=240)
                if cand:
                    scan_s=5.0*len(self.unknown())+max(0,len(self.unknown())-1)
                    power=max(0.05,float(self.g("explore_cost_ratio_pd_power")))
                    best=None
                    for p in cand:
                        p=np.asarray(p,float);pd=self.cover.conditional_detect(p)
                        cost=float(np.linalg.norm(p-cur))/5.0+scan_s
                        score=(max(1e-9,pd)**power)/max(1.0,cost)
                        rec=(score,pd,-cost,p)
                        if best is None or rec[:3]>best[:3]:best=rec
                    return best[3]
        ...
        if not cand:return None
        thr=self.stop_threshold(k) if target_thr is None else float(target_thr);ps0=self.posterior_same_count();m=len(self.unknown())
        ...
        return best[0]
    def exploration_point(self,target_thr=None):
        if int(self.g("channel_posterior_enabled")) and int(self.g("channel_explore_scan_limit"))>0:return self._channel_exploration_point()
        ...
        return self._exploration_point_base(target_thr=target_thr)
    @staticmethod
    def _elementary(values):
        e=[1.0]
        for v in values:
            e.append(0.0)
            for r in range(len(e)-1,0,-1):e[r]+=float(v)*e[r-1]
        return e
    def _channel_posterior_state(self):
        unknown=self.unknown();k=self.known();m=len(unknown)
        if not unknown:return {},{k:1.0}
        miss=[]
        for ch in unknown:
            d=np.sqrt(self.channel_d2[ch].astype(float));w=np.ones(len(d));w[d<=1000.0]=0.0
            z=(d>1000.0)&(d<1500.0);w[z]=(d[z]-1000.0)/500.0
            miss.append(max(1e-10,float(np.mean(w))))
        e=self._elementary(miss);raw={};zsum=0.0
        for n in range(max(10,k),MAXN+1):
            r=n-k
            if 0<=r<=m:
                w=e[r]/float(math.comb(20,n));raw[n]=w;zsum+=w
        if zsum<=0:return {ch:0.0 for ch in unknown},{}
        dist={n:w/zsum for n,w in raw.items()}
        marg={}
        for i,ch in enumerate(unknown):
            ex=self._elementary(miss[:i]+miss[i+1:]);num=0.0
            for n in raw:
                r=n-k
                if 1<=r<=m:num+=miss[i]*ex[r-1]/float(math.comb(20,n))
            marg[ch]=max(0.0,min(1.0,num/zsum))
        return marg,dist
    def _channel_conditional_detect(self,ch,p):
        dprev=np.sqrt(self.channel_d2[ch].astype(float));w=np.ones(len(dprev));w[dprev<=1000.0]=0.0
        z=(dprev>1000.0)&(dprev<1500.0);w[z]=(dprev[z]-1000.0)/500.0
        den=float(np.sum(w))
        if den<=1e-12:return 0.0
        dn=np.linalg.norm(self.cover.p-np.asarray(p,float),axis=1)
        joint=np.maximum(0.0,np.minimum(dprev,1500.0)-np.maximum(dn,1000.0))/500.0
        return max(0.0,min(1.0,float(np.sum(joint)/den)))
    def channel_detect_scores(self,p):
        marg,_=self._channel_posterior_state()
        return [(ch,marg.get(ch,0.0)*self._channel_conditional_detect(ch,p)) for ch in self.unknown()]
    def channel_detect_any(self,p,limit=None):
        vals=sorted((v for _,v in self.channel_detect_scores(p)),reverse=True)
        if limit is not None and int(limit)>0:vals=vals[:int(limit)]
        miss=1.0
        for v in vals:miss*=1.0-max(0.0,min(1.0,v))
        return 1.0-miss
    def _channel_exploration_point(self):
        cur=np.asarray(self.c.position,float);cand=self._explore_candidates(cur,max_pool=int(self.g("channel_candidate_pool")))
        if not cand:return None
        limit=int(self.g("channel_explore_scan_limit"));best=None
        for p in cand:
            p=np.asarray(p,float);pd=self.channel_detect_any(p,limit=limit);dist=float(np.linalg.norm(p-cur))
            rec=(pd-dist/float(self.g("channel_probe_dist_div")),pd,-dist,p)
            if best is None or rec[:3]>best[:3]:best=rec
        return best[3]

    def count_posterior(self):
        if int(self.g("channel_count_posterior_enabled")):return self._channel_posterior_state()[1]
        k=self.known();q=max(1e-6,min(1-1e-9,self.cover.expected_detect()));ws=[]
        for n in range(max(10,k),MAXN+1):
            ws.append((n,math.comb(n,k)*(q**k)*((1-q)**(n-k))))
        z=sum(w for _,w in ws)
        return {n:w/z for n,w in ws} if z>0 else {}
    def posterior_same_count(self):
        k=self.known()
        if k<10:return 0.0
        return float(self.count_posterior().get(k,0.0))
    def posterior_p16(self):
        return float(self.count_posterior().get(MAXN,0.0))
    ...
    def run(self):
        ...
        try:
            ...
            if int(self.g("bootstrap_unknown_scan")):
                blim=int(self.g("bootstrap_unknown_limit"))
                if blim>0 and blim<len(self.unknown()):
                    print(f"[启动] 第二测站前期精简扫描 {blim}/{len(self.unknown())} 个频道，余量留给动态补测",flush=True)
                    before_unknown=set(self.unknown())
                    self.scan_unknown_partial(p,blim)
                    if int(self.g("deferred_blind_scan")):
                        measured_here={ch for ch in before_unknown if any(np.linalg.norm(h[0]-p)<1 for h in self.B[ch].hist)}
                        self.deferred_bootstrap_channels={ch for ch in before_unknown-measured_here if self.B[ch].status=='unknown'}
                        print(f"[启动] 欠测频道登记 {len(self.deferred_bootstrap_channels)} 个，沿清除路线向盲区补扫",flush=True)
                else:self.scan_unknown(p)
            ...
            for front_i in range(max(0,int(self.g("frontload_probe_count")))):
                if self.known()>=MAXN:break
                ep=self.exploration_point()
                if ep is None:break
                print(f"[前置发现] 第{front_i+1}个后验测站 {self._pos(ep)}，先扩充已知源再规划清除路线",flush=True)
                self.scan_unknown(ep)
                lim=int(self.g("frontload_probe_info_limit"))
            ...
            for step in range(120):
                ...
                if (detected and int(self.g("midroute_discovery_probe")) and not self.midroute_probe_used
                    and int(self.g("midroute_probe_min_known"))<=k<=int(self.g("midroute_probe_max_known"))
                    and len(self.cleared)>=int(self.g("midroute_probe_min_cleared"))
                    and len(detected)>=int(self.g("midroute_probe_min_pending"))):
                    ep=self.exploration_point();self.midroute_probe_used=True
                    if ep is not None:
                        print(f"[中段找补] 已发现{k}且至少还缺{max(1,10-k)}源，提前普查 {self._pos(ep)}",flush=True)
                ...
                if detected:
                    ...
                    if kind=='probe':
                        terminal=bool(self.executing_terminal_probe)
                        print((f"[路线内终检] 执行虚拟验证点 {self._pos(p)}" if terminal else f"[探索] 插入后验探测点 {self._pos(p)}，继续寻找未发现源"),flush=True)
                        gain=self.scan_unknown(p);self.scan_info(p,limit=int(self.g("probe_info_limit")))
                        if terminal:
                            self.terminal_probe_used=True;self.terminal_probe_empty=(gain==0);self.executing_terminal_probe=False;self.terminal_probe=None
                            print(f"[路线内终检] {'未发现新源' if gain==0 else '发现新源并重建路线'}",flush=True)
                        continue
                    ...
                    if not hard_done and not soft_done:
                        pd=(self.channel_detect_any(q,limit=int(self.g("route_scan_partial_limit")))
                            if int(self.g("channel_posterior_enabled")) else self.cover.conditional_detect(q))
                        ...
                        if use_adaptive:
                            ...
                            scan_value=float(self.g("route_scan_value_s"))
                            if (int(self.g("route_scan_lowp16_enabled")) and self.known()>=int(self.g("route_scan_lowp16_min_known"))
                                and self.posterior_p16()<=float(self.g("route_scan_lowp16_cut"))):
                                scan_value=float(self.g("route_scan_lowp16_value_s"))
                            thscan=scan_s/max(1.0,scan_value*hp)
                        else:
                        ...
                        thscan=max(thscan,float(self.g("route_scan_min_pd")))
                        if self.known()>=int(self.g("route_scan_high_known_min_known")):
                            thscan=max(thscan,float(self.g("route_scan_high_known_floor")))
                        if pd>=thscan:
                            print(f"[顺路普查] 当前清除点仍有发现隐藏源价值 P≈{pd:.2f}",flush=True)
                            deferred=(int(self.g("deferred_blind_scan")) and self.deferred_bootstrap_channels
                                and self.known()<=int(self.g("deferred_blind_max_known"))
                                and pd>=float(self.g("deferred_blind_min_pd")))
                            if deferred:
                                self.scan_deferred_blind(q,int(self.g("deferred_blind_batch")))
                            else:
                                lim=int(self.g("route_scan_partial_limit"))
                                p16_min=float(self.g("route_scan_partial_p16_min"))
                                p16_ok=(p16_min<0.0 or self.posterior_p16()>=p16_min)
                                if (lim>0 and self.known()>=int(self.g("route_scan_partial_min_known")) and p16_ok
                                    and lim<len(self.unknown()) and pd<float(self.g("route_scan_partial_below_pd"))):
                                    self.scan_unknown_partial(q,lim)
                                else:
                                    self.scan_unknown(q)
                ...
                if self.terminal_probe_empty and k>=10 and len(self.cleared)>=k:
                    ratio=float(self.g("terminal_probe_empty_accept_ratio"));boundary_ok=(k<15 or self.posterior_p16()<=float(self.g("terminal_probe_empty_p16_max")))
                    if boundary_ok and ps>=ratio*thr:
                        print(f"[路线内终检] 空扫描已吸收进清除路线，后验={ps:.3f}，结束",flush=True);break
                if discovery_done:
                    target=float(self.g("soft_stop_target"))
                    need_terminal=(target<0.0 or self.posterior_same_count()<target)
                    if soft_done and need_terminal and self.soft_stop_probes < int(self.g("soft_stop_probe_limit")):
                        ep=self.exploration_point(target_thr=(target if target>=0.0 else None))
                        self.soft_stop_probes += 1
                        if ep is not None:
                            print(f"[结束前复查] 第{self.soft_stop_probes}次独立后验测量 {self._pos(ep)}，目标后验={target:.3f}",flush=True)
                ...
                clim=int(self.g("channel_explore_scan_limit"))
                gain=(self.scan_unknown_partial(ep,clim) if int(self.g("channel_posterior_enabled")) and clim>0 else self.scan_unknown(ep))
