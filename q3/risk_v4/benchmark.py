...
sys.path[:0] = [str(ROOT / 'src'), str(BASE / 'src'), str(BASE / 'simulator')]
...
def fingerprints():
    files = [Path(__file__), *sorted((ROOT / 'src').glob('*.py')),
             *sorted((BASE / 'src').glob('*.py')), *sorted((BASE / 'simulator').glob('*.py')),
             *sorted((ROOT / 'configs').glob('*.json'))]
    return {Path(os.path.relpath(p, ROOT)).as_posix(): hashlib.sha256(p.read_bytes()).hexdigest() for p in files}
...
def child(variant, seed, output):
    ...
    if controller is not None:
        for key in ('completion_verified', 'completion_reason', 'completion_certificate', 'geometry_report', 'risk_report'):
...
def aggregate(rows):
    ...
    for name in sorted({r['variant'] for r in rows}):
        ...
        failure_count = len(data) - len(ok)
        upper95 = 1.0 if failure_count == len(data) else float(beta.ppf(0.95, failure_count + 1, len(ok)))
        timed = [r for r in data if r.get('avg_clear_time_s') is not None]
        out.append({'variant': name, 'runs': len(data), 'fullclear': len(ok),
                    'failure_rate': failure_count / len(data), 'failure_rate_upper95': upper95,
                    'all_case_mean_s_per_cleared_source': statistics.mean(r['avg_clear_time_s'] for r in timed) if len(timed) == len(data) else None,
...
def main():
    ...
    parser.add_argument('--variants', default='certified_reference,risk001')
    ...
    hashes = fingerprints()
    if hashes['../geometry_v3/simulator/jammers_offline_sim.py'] != SIM_HASH:
    ...
    manifest = {'created_at': datetime.now(timezone.utc).isoformat(), 'seeds': seeds,
                ...
                'metric': 'Report all-case time / actually cleared count plus run-level failure and one-sided 95% Clopper-Pearson upper limit; never hide failed cases',
