...
VERSION='v448-defer-last-bootstrap-at15'
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    if BOOTSTRAP_POINTS>1:
        bootstrap_angles=np.arange(BOOTSTRAP_POINTS-1)*360/max(1,BOOTSTRAP_POINTS-1)
        for bootstrap_index,a in enumerate(bootstrap_angles):
            q=800*np.asarray([math.cos(math.radians(a)),math.sin(math.radians(a))])
            if bootstrap_index==len(bootstrap_angles)-1 and len(found)>=15:
                pending_probes.append(q)
                break
            scan(q,force=True)
