...
VERSION='v440-deferred-fifth-joint-route'
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    def clear_detected(attempt_limit=2):
        while time.monotonic()-start<deadline:
            options=[]
            for c in sorted(found):
                b=beliefs[c]
                if b['status']!='detected' or attempted.get(c,0)>=attempt_limit:continue
    ...
    # 先走前三个800米环点；最后一个环点保留为必访任务，与首批清除目标统一排程。
    ring_angles=np.arange(BOOTSTRAP_POINTS-1)*360/max(1,BOOTSTRAP_POINTS-1)
    for a in ring_angles[:-1]:
        q=800*np.asarray([math.cos(math.radians(a)),math.sin(math.radians(a))])
        scan(q,force=True)
    if len(ring_angles):
        a=float(ring_angles[-1])
        pending_probes.append(800*np.asarray([math.cos(math.radians(a)),math.sin(math.radians(a))]))
    # 第五点前每个频道最多尝试一次；失败源等待第五点或沿途扫描补充测向后再试。
    clear_detected(attempt_limit=1)
    # 第五点已经完成后，再按更新后的公开belief规划联合探点。
    ...
    clear_detected(attempt_limit=2)
    tails=[]
