...
def select_probe(belief, current, used, travel_penalty=0.00035):
    ...
    for ring_radius in (0.0, 500.0, 1000.0, 1400.0, 1750.0, 1950.0, 2100.0):
...
def rank_radial_shadow_probes(beliefs,current,ring_radius=2100.0,min_inner_radius=1400.0):
    """Generate robust outward extensions from shared unknown-channel no-signal points."""
    buckets={}
    for belief in beliefs.values():
        if belief.get('status')!='unknown':
            continue
        seen=set()
        for point in belief.get('no_signal_points',[]):
            point=np.asarray(point,float); radius=float(np.linalg.norm(point))
            if radius<min_inner_radius:
                continue
            angle=math.degrees(math.atan2(float(point[1]),float(point[0])))%360.0
            key=int(round(angle/5.0))%72
            if key in seen:
                continue
            seen.add(key); buckets.setdefault(key,[]).append((angle,radius))
    candidates=[]
    for items in buckets.values():
        angle=float(np.mean([item[0] for item in items])); a=math.radians(angle)
        point=ring_radius*np.asarray([math.cos(a),math.sin(a)])
        candidates.append({'support':len(items),'inner_radius':max(item[1] for item in items),
                           'angle_deg':angle,'point':point})
    return candidates

def plan_farthest_shadow_pairs(beliefs,current,ring_radius=2100.0,budget=6):
    """Cover long radial shadows first; pair +45 degree and radial observers."""
    bases=[item for item in rank_radial_shadow_probes(beliefs,current,ring_radius=ring_radius)
           if item['inner_radius']<=1775.0]
    route=[]; cursor=np.asarray(current,float); used_angles=[]
    while bases and len(route)<budget:
        min_radius=min(item['inner_radius'] for item in bases)
        layer=[item for item in bases if item['inner_radius']<=min_radius+30.0]
        base=max(layer,key=lambda item:float(np.linalg.norm(item['point']-cursor)))
        for angle in ((base['angle_deg']+45.0)%360.0,base['angle_deg']):
            if len(route)>=budget:
                break
            if any(min(abs(angle-a),360.0-abs(angle-a))<12.0 for a in used_angles):
                continue
            rad=math.radians(angle); point=ring_radius*np.asarray([math.cos(rad),math.sin(rad)])
            route.append({'support':base['support'],'inner_radius':base['inner_radius'],
                          'base_angle_deg':base['angle_deg'],'angle_deg':angle,'point':point})
            used_angles.append(angle); cursor=point
        chosen=base['angle_deg']
        bases=[item for item in bases
               if min(abs(item['angle_deg']-chosen),360.0-abs(item['angle_deg']-chosen))>=20.0]
    return route
