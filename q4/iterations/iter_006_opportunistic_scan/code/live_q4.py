...
VERSION = "v394-opportunistic-scan"
DENSE_SWITCH_MIN = 14
...
def run_q4(client: SimulatorClient, *, case_code: str, real_margin_s: float = 45.0,
    ...
    try:
        ...
        def rescue_unknown_here() -> None:
            ...
            if not unknown:
                return
            # Every remaining unknown channel has only public negative observations.
            # Scan at an existing clear endpoint only if the conditional visibility
            # gain is material; this action adds no travel to the clear route.
            from q4_targeted_rescue_v3 import point_visibility_probability
            probability = point_visibility_probability(beliefs[unknown[0]], here)
            if probability < 0.05:
                return
            rescue_sites.append(here.copy())
            label = f"原地机会(预测可见率{probability:.3f})"
            unknown = [c for c,b in beliefs.items() if b.get("status") == "unknown"]
        ...
        if TARGETED_POST_CLEAR_RESCUE and _targeted_trigger:
            ...
            for _target_index in range(_target_budget):
                ...
                # 补查总预算为两次；空探针只更新后验，不作为不存在剩余源的证明。
