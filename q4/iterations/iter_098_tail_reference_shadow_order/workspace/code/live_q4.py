...
VERSION='v464-tail-reference-shadow-order'
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    if shadow_unknown and len(found)<16:
        ...
        if shadow_trigger:
            from q4_targeted_rescue_v3 import plan_farthest_shadow_pairs
            print(f'径向阴影找补启动 已发现{len(found)} 风险{shadow_risk:.3f} 内环{_p16:.3f} 外环{_p21:.3f}')
            shadow_reference=np.asarray(tails[-1],float) if tails else client.current_position_m
            route=plan_farthest_shadow_pairs(beliefs,shadow_reference,
                ring_radius=SHADOW_RESCUE_RING_RADIUS_M,budget=SHADOW_RESCUE_BUDGET)
            for k,item in enumerate(route):
                if len(found)>=16 or time.monotonic()-start>=deadline:
                    break
                q=np.asarray(item['point'],float); before=len(found)
                print(f"远端阴影对{k+1} 角度{item['angle_deg']:.1f} 基准{item['base_angle_deg']:.1f}")
                scan(q,force=True); tails.append(q.tolist()); clear_detected()
                if len(found)>before:
                    print('径向阴影首次命中，立即停止外环找补')
                    break
    client.exit()
