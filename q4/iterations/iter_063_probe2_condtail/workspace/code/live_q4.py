...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    clear_detected()
    tails=[]
    previous_tail_found = True
    for k in range(TAIL_BUDGET):
        if k > 0 and not previous_tail_found: break
        ...
        before_tail_found=len(found)
        scan(q,force=True);tails.append(q.tolist())
        previous_tail_found=(len(found)>before_tail_found)
        clear_detected()
    client.exit()
