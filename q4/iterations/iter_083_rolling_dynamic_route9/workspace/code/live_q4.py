...
VERSION='v450-rolling-dynamic-route9'
...
DYNAMIC_ROUTE_MAX_PROBES=4
DYNAMIC_ROUTE_MIN_COVERAGE=0.08
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    dense_dynamic_route=False
    dynamic_probe_uses=0
    ...
    def scan(point, force=False):
        ...
        if not force and unknown and len(found)>=10:
            if remaining_active_risk(beliefs,len(found))<=OPPORTUNITY_REMAINING_RISK_FLOOR:
                unknown=[]
    ...
    def refresh_dynamic_probe():
        """只保留基于最新公开反馈生成的下一探点。"""
        nonlocal dynamic_probe_uses
        pending_probes.clear()
        if not dense_dynamic_route or dynamic_probe_uses>=DYNAMIC_ROUTE_MAX_PROBES or len(found)>=16:
            return
        unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
        if not unknown:
            return
        hypothetical=[core._belief_target_point(beliefs[c]) for c in sorted(found) if beliefs[c]['status']=='detected']
        planned,stats=plan_probes_with_stats(
            beliefs[unknown[0]],client.current_position_m,
            [x for x in hypothetical if x is not None],4)
        for point,stat in zip(planned,stats):
            if visited and min(np.linalg.norm(np.asarray(point)-np.asarray(v)) for v in visited)<MIN_SCAN_SEPARATION:
                continue
            if stat['coverage']<DYNAMIC_ROUTE_MIN_COVERAGE and stat['insertion_m']>250.0:
                continue
            pending_probes.append(point)
            return

    def clear_detected():
        nonlocal dynamic_probe_uses
        while time.monotonic()-start<deadline:
            if dense_dynamic_route:
                refresh_dynamic_probe()
            options=[]
    ...
    hypothetical=[core._belief_target_point(beliefs[c]) for c in sorted(found) if beliefs[c]['status']=='detected']
    unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
    if unknown and not dense_dynamic_route:
    ...
    for k in range(TAIL_BUDGET):
        ...
        if len(found)>=10 and remaining_active_risk(beliefs,len(found))<=TAIL_REMAINING_RISK_FLOOR:
