...
VERSION='v454-evidence-reopens-clear'
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    beliefs=core.new_beliefs(); found=set(); cleared=set(); visited=[]; attempted_since={}; attempted_total={}; pending_probes=[]
    ...
    def scan(point, force=False):
        ...
        for c in channels:
            evidence_before=(len(beliefs[c].get('directions',[])),len(beliefs[c].get('no_signal_points',[])))
            body=core._measure_live(client,beliefs,point,c)
            evidence_after=(len(beliefs[c].get('directions',[])),len(beliefs[c].get('no_signal_points',[])))
    ...
    def clear_detected():
        while time.monotonic()-start<deadline:
            ...
            for c in sorted(found):
                b=beliefs[c]
                if (b['status']!='detected' or attempted_since.get(c,0)>=2
                        or attempted_total.get(c,0)>=4):continue
            ...
            evidence_before=(len(beliefs[c].get('directions',[])),len(beliefs[c].get('no_signal_points',[])))
            attempted_since[c]=attempted_since.get(c,0)+1
            attempted_total[c]=attempted_total.get(c,0)+1
    ...
    if BOOTSTRAP_POINTS>1 and not dense_dynamic_route:
        ...
        for a in np.arange(BOOTSTRAP_POINTS-1)*360/max(1,BOOTSTRAP_POINTS-1):
            bootstrap.append(800*np.asarray([math.cos(math.radians(a)),math.sin(math.radians(a))]))
        for q in bootstrap[:2]:
            scan(q,force=True)
        pending_probes.extend(bootstrap[2:])
