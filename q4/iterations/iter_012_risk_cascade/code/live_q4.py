...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    for k in range(TAIL_BUDGET):
        ...
        q,p,risk=choose_probe(beliefs[unknown[0]],client.current_position_m,[np.asarray(x) for x in visited],len(found),len(unknown))
        print(f'剩余源风险估计{risk:.5f}')
        if len(found)>=10 and risk<0.03:break
        if q is None:break
