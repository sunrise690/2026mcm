...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
    if unknown:
        pending_probes.extend(plan_probes(beliefs[unknown[0]],client.current_position_m,[x for x in hypothetical if x is not None],2))
    clear_detected()
    risk,target_channel=remaining_risk(beliefs,found)
    unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
    if unknown and risk>=STOP_RISK:
        target_channel=target_channel if target_channel in unknown else unknown[0]
        hypothetical=[core._belief_target_point(beliefs[c]) for c in sorted(found) if beliefs[c]['status']=='detected']
        pending_probes.extend(plan_probes(beliefs[target_channel],client.current_position_m,[x for x in hypothetical if x is not None],2))
        print(f'频道后验剩余风险{risk:.3f}，执行第二批联合探点')
        clear_detected()
    else:
        print(f'频道后验剩余风险{risk:.3f}，省略第二批联合探点')
    tails=[]
    for k in range(TAIL_BUDGET):
        if len(found)>=16 or time.monotonic()-start>=deadline:break
        unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
        if not unknown:break
        risk,_=remaining_risk(beliefs,found)
        if risk<STOP_RISK:
