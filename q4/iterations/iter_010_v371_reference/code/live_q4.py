...
VERSION = "v371-cloud-start"
DENSE_SWITCH_MIN = 1
DENSE_SWITCH_AFTER_INDEX = 4
DENSE_CONFIRM_POINTS = 0
MAX_RESCUE_SITES = 16
RESCUE_MIN_SEPARATION_M = 300.0
TARGETED_POST_CLEAR_RESCUE = True
TARGETED_POST_CLEAR_BUDGET = 5
TARGETED_BASE_TRIGGER_MIN_POSITIVE = 0
TARGETED_FORCE_MIN_POSITIVE = 0
TARGETED_FORCE_MAX_BUDGET = 5
TARGETED_RESCUE_REQUIRE_PRIOR_FIND = False
TARGETED_RESCUE_MIN_STOP_COUNT = 1
TARGETED_RESCUE_TRAVEL_PENALTY = 0.0012
TARGETED_BREAK_ON_EMPTY = True
TARGETED_REQUIRED_MIN_POSITIVE = 10
TARGETED_LOW_COUNT_TRAVEL_PENALTY = 0.00065
TARGETED_AFTER_FIND_TRAVEL_PENALTY = 0.0009
TARGETED_LOW_COUNT_MAX_EMPTY = 2
TARGETED_EMPTY_STREAK_STOP = 2
TARGETED_GAP_TAIL_ENABLE = True
TARGETED_GAP_RING_RADIUS_M = 1750.0
TARGETED_GAP_TRAVEL_PENALTY = 0.00025
...
MASTER_INNER_RADIUS_M = 850.0
MASTER_OUTER_RADIUS_M = 1600.0
MASTER_INNER_POINTS = 4
...
def run_q4(client: SimulatorClient, *, case_code: str, real_margin_s: float = 45.0,
    ...
    if fast:
        ...
        if CENSUS_ROUTE_MODE == "single_ring":
    ...
    try:
        ...
        for route_index, point in enumerate(route_points):
            ...
            if scan_detected_again:
                _rescans = q4_select_rescan_channels(beliefs, point)
                # v341c: third bearings are useful only once the case is clearly
                # dense; before then keep the v321 two-bearing economy.
        ...
        if TARGETED_POST_CLEAR_RESCUE and _targeted_trigger:
            ...
            for _target_index in range(_target_budget):
                ...
                # v371: choose a cheap Bayesian probe first.  If the previous probe was
                # empty, force angular diversity on the outer ring to attack boundary
                # directional-source blind crescents instead of greedily staying nearby.
                _used_all = [np.asarray(x, float) for x in visited] + rescue_sites + targeted_sites
                if TARGETED_GAP_TAIL_ENABLE and targeted_sites and _target_empty_streak > 0:
                    _angles_used = []
                    for _u in _used_all:
                        if float(np.linalg.norm(_u)) >= 700.0:
                            _angles_used.append(math.atan2(float(_u[1]), float(_u[0])))
                    _best_gap = None
                    _ring = float(TARGETED_GAP_RING_RADIUS_M)
                    for _deg in np.arange(0.0, 360.0, 10.0):
                        _a = math.radians(float(_deg))
                        _qq = _ring * np.asarray([math.cos(_a), math.sin(_a)], float)
                        if _angles_used:
                            _gap = min(abs(math.atan2(math.sin(_a-u), math.cos(_a-u))) for u in _angles_used)
                        else:
                            _gap = math.pi
                        _travel = float(np.linalg.norm(_qq - np.asarray(client.current_position_m, float)))
                        _score = _gap - float(TARGETED_GAP_TRAVEL_PENALTY) * _travel
                        _item = (_score, _gap, -_travel, _qq)
                        if _best_gap is None or _item[:3] > _best_gap[:3]:
                            _best_gap = _item
                    q = None if _best_gap is None else np.asarray(_best_gap[3], float)
                    probability = 0.0
                else:
                    if len(positive_channels) < TARGETED_REQUIRED_MIN_POSITIVE:
                        _probe_penalty = float(TARGETED_LOW_COUNT_TRAVEL_PENALTY)
                    elif _last_found:
                        _probe_penalty = float(TARGETED_AFTER_FIND_TRAVEL_PENALTY)
                    else:
                        _probe_penalty = float(TARGETED_RESCUE_TRAVEL_PENALTY)
                ...
                if found:
                    rescue_discovery_count += len(found)
                    _target_empty_streak = 0
                    _last_found = True
                else:
                ...
                # Newly found sources create useful, widely scattered sensor locations.
                ...
                while True:
                    ...
                    valid = [(c, q0) for c, q0 in zip(candidates, targets) if q0 is not None]
                    if not valid:
                        break
                ...
                _empty_limit = (int(TARGETED_LOW_COUNT_MAX_EMPTY)
                                if len(positive_channels) < TARGETED_REQUIRED_MIN_POSITIVE
                                else int(TARGETED_EMPTY_STREAK_STOP))
                if TARGETED_BREAK_ON_EMPTY and _target_empty_streak >= max(1, _empty_limit):
                    break
    ...
    summary = {
        ...
        "note": "v371 云端起点：4点850m前缀、自适应后验补漏、连续空扫停止、外缘角隙风险补查。",
    }
