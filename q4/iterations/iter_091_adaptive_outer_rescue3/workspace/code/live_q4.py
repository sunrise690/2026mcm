...
VERSION='v458-adaptive-outer-rescue3'
...
ADAPTIVE_OUTER_RESCUE_BUDGET=3
ADAPTIVE_OUTER_RESCUE_RISK_FLOOR=0.20
ADAPTIVE_OUTER_RESCUE_RING_RADIUS_M=1600.0
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    for k in range(TAIL_BUDGET):
        ...
        clear_detected()
    # 难例动态找补：每次外圈扫描后重新计算残余后验，再选下一点。
    from q4_targeted_rescue_v3 import select_state_tail_probe
    for k in range(ADAPTIVE_OUTER_RESCUE_BUDGET):
        if len(found)>=16 or time.monotonic()-start>=deadline:break
        unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
        if not unknown:break
        risk=remaining_active_risk(beliefs,len(found)) if len(found)>=10 else 1.0
        if risk<=ADAPTIVE_OUTER_RESCUE_RISK_FLOOR:break
        q,p=select_state_tail_probe(beliefs[unknown[0]],client.current_position_m,mode='balanced',
            ring_radius=ADAPTIVE_OUTER_RESCUE_RING_RADIUS_M,travel_penalty=0.00012)
        if q is None or (visited and min(np.linalg.norm(q-np.asarray(x)) for x in visited)<MIN_SCAN_SEPARATION):break
        print(f'动态外圈找补{k+1} 风险{risk:.3f} 覆盖{p:.3f}')
        scan(q,force=True);tails.append(q.tolist())
        clear_detected()
    client.exit()
