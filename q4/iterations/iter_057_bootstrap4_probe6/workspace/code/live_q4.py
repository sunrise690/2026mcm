...
VERSION='v432-bootstrap4-probe6'
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
    if unknown:
        pending_probes.extend(plan_probes(beliefs[unknown[0]],client.current_position_m,[x for x in hypothetical if x is not None],6))
    clear_detected()
    tails=[]
