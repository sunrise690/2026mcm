...
def run_q4(client: SimulatorClient, *, case_code: str, real_margin_s: float = 45.0,
    ...
    low_plateau_count: int | None = None
    low_plateau_index: int | None = None
    ...
    try:
        ...
        for route_index, point in enumerate(route_points):
            ...
            # 低密度公开计数平台：连续2个普查间隔无新增后，提前转入清除路径；
            # 后续仍保留沿途 unknown 扫描和完整后验补漏。
            if 10 <= len(positive_channels) <= 13 and route_index >= 10:
                count_now = len(positive_channels)
                if low_plateau_count != count_now:
                    low_plateau_count = count_now
                    low_plateau_index = route_index
                elif low_plateau_index is not None and route_index - low_plateau_index >= 2:
                    high14_early_stop = True
                    density_stop_count = count_now
                    print(f"低密度计数平台提前切换：第{route_index+1}点已发现{count_now}源。")
                    break
