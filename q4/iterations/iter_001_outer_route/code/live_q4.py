...
TARGETED_AFTER_FIND_TRAVEL_PENALTY = 0.00090
...
def run_q4(client: SimulatorClient, *, case_code: str, real_margin_s: float = 45.0,
    ...
    try:
        ...
        _state_tail_suspect = ((len(positive_channels) == 13 and len(visited) >= 5)
                               or (len(positive_channels) == 14 and len(visited) >= 8))
        ...
        if TARGETED_POST_CLEAR_RESCUE and _targeted_trigger:
            ...
            for _target_index in range(_target_budget):
                ...
                if targeted_sites and _target_empty_streak > 0:
                    # Risk tail for edge directional sources: choose the least-covered
                    # angular sector on the outer ring instead of another local greedy probe.
                    _angles_used=[]
                    for _u in _used_all:
                    ...
                    for _deg in np.arange(0.0,360.0,10.0):
                        ...
                        _qq=1750.0*np.asarray([math.cos(_a),math.sin(_a)],float)
                        if _angles_used:
                        ...
                        _score=_gap-0.00025*_travel
                        _item=(_score,_gap,-_travel,_qq)
                        if _best is None or _item[:3]>_best[:3]: _best=_item
                    q=np.asarray(_best[3],float) if _best is not None else None
                    probability=0.0
                else:
                ...
                # v361 adaptive tail: once the lower bound N>=10 has been met,
                # an empty posterior probe is strong evidence that the remaining
                # unknown channels are mostly inactive.  Stop immediately instead
                # of paying for several long ghost-search moves.  Below 10 sources
                # we must keep searching, but cap consecutive empty probes.
                if found:
                    _target_empty_streak = 0
                else:
        ...
        _tail_mode = None
        if len(positive_channels) == 13 and len(visited) >= 5:
            _tail_mode = "mean"
        elif len(positive_channels) == 14 and len(visited) >= 8:
            _tail_mode = "balanced"
