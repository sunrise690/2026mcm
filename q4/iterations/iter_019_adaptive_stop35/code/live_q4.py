...
from risk_rescue import choose_probe

VERSION='v403-adaptive-stop35'
STOP_RISK=0.35
TAIL_BUDGET=1
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    def clear_detected():
        while time.monotonic()-start<deadline:
            ...
            if index>=len(options):
                unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
                if len(found)>=10 and unknown:
                    _,_,risk=choose_probe(beliefs[unknown[0]],client.current_position_m,[np.asarray(x) for x in visited],len(found),len(unknown))
                    if risk<STOP_RISK:
                        print(f'剩余源风险{risk:.3f}低于阈值，停止联合补探')
    ...
    for k in range(TAIL_BUDGET):
        ...
        if len(found)>=10:
            _,_,risk=choose_probe(beliefs[unknown[0]],client.current_position_m,[np.asarray(x) for x in visited],len(found),len(unknown))
            if risk<STOP_RISK:
                print(f'尾部剩余源风险{risk:.3f}低于阈值，停止补探')
                break
