...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    def scan(point, force=False):
        ...
        frontier=min(16,lower+FRONTIER_WIDTH)
        unknown=[c for c,b in beliefs.items() if b['status']=='unknown' and c<=frontier]
        ...
        while index<len(channels):
            ...
            if body.get('measure_result') in ('direction','near'):
                ...
                new_frontier=min(16,max(10,max(found))+FRONTIER_WIDTH)
                for extra in range(1,new_frontier+1):
                    if beliefs[extra]['status']=='unknown' and extra not in channels:
    ...
    if unknown:
        pending_probes.extend(plan_probes(beliefs[unknown[0]],client.current_position_m,[x for x in hypothetical if x is not None],4))
    clear_detected()
    tails=[]
