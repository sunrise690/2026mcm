...
VERSION='v466-reported-opposite-outer-single'
...
OPPOSITE_RESCUE_RING_RADIUS_M=2100.0
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    if not value_rescue_moved:
        for k in range(ADAPTIVE_RESCUE_BUDGET):
            ...
            scan(q,force=True);tails.append(q.tolist())
            clear_detected()
    # 外向定向源可能在最后尾探的对侧外缘；仅在内环明显失效时做一次外侧补点。
    opposite_unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
    if opposite_unknown and len(found)<16 and tails:
        opposite_risk=remaining_active_risk(beliefs,len(found)) if len(found)>=10 else 1.0
        from q4_targeted_rescue_v3 import select_state_tail_probe
        _q16,_p16=select_state_tail_probe(beliefs[opposite_unknown[0]],client.current_position_m,
            mode='balanced',ring_radius=1600.0,travel_penalty=SECOND_RESCUE_TRAVEL_PENALTY)
        _q21,_p21=select_state_tail_probe(beliefs[opposite_unknown[0]],client.current_position_m,
            mode='balanced',ring_radius=OPPOSITE_RESCUE_RING_RADIUS_M,travel_penalty=SECOND_RESCUE_TRAVEL_PENALTY)
        opposite_trigger=(len(found)<10
            or (len(found)==12 and opposite_risk>=0.30 and _p16<0.10)
            or (len(found)==14 and _p16<0.08)
            or (len(found)==14 and opposite_risk<0.22 and _p16<0.10 and _p21>0.28))
        if opposite_trigger:
            tail=np.asarray(tails[-1],float)
            tail_angle=math.degrees(math.atan2(float(tail[1]),float(tail[0])))%360.0
            offset=180.0 if len(found)<10 else 210.0
            angle=(tail_angle+offset)%360.0; a=math.radians(angle)
            q=OPPOSITE_RESCUE_RING_RADIUS_M*np.asarray([math.cos(a),math.sin(a)])
            print(f'对侧外环单点 已发现{len(found)} 尾探{tail_angle:.1f} 补点{angle:.1f}')
            scan(q,force=True);tails.append(q.tolist());clear_detected()
    client.exit()
    ...
    average_time=result['average_clear_time_s_observed_by_program']
    print('\n=== 第四问运行摘要 ===')
    print(f"程序版本: {VERSION}")
    print(f"已发现频道: {len(found)}")
    print(f"已清除频道: {len(cleared)}")
    print(f"总虚拟时间: {client.current_virtual_time_s:.3f} s")
    print(f"平均清除时间: {average_time:.3f} s/源" if average_time is not None else "平均清除时间: N/A")
    print(f"扫描位置数: {len(visited)}")
    print(f"动态找补点数: {len(tails)}")
    print(f"退出时未知频道数: {len(result['unknown_channels_at_exit'])}")
    print('========================')
