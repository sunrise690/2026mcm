...
def select_probe(belief, current, used, travel_penalty=0.00035, *, unknown_count=0, max_cost_s=None, min_score=None):
    pos, radius, heading_u, directional = _particles(belief.get("no_signal_points", []))
    belief["rescue_particle_survival"] = float(len(pos)) / 200000.0
    if not len(pos):
        return None, 0.0
    ...
    for ring_radius in (0.0, 500.0, 1000.0, 1400.0, 1750.0, 1900.0, 2100.0):
    ...
    for local_radius in (350.0, 650.0, 900.0):
        for angle_deg in np.arange(0.0, 360.0, 20.0):
            ...
            if float(np.linalg.norm(point)) <= 2100.0 + 1e-9:
    ...
    for point in candidates:
        ...
        action_cost_s = 6.0 * int(unknown_count)
        if max_cost_s is not None and travel_s + action_cost_s > float(max_cost_s):
            continue
        score = probability - travel_penalty * travel_s
        if min_score is not None and score < float(min_score):
            continue
        item = (score, probability, -travel_s, point)
