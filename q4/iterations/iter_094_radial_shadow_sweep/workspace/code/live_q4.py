...
VERSION='v460-radial-shadow-sweep'
...
BOOTSTRAP_POINTS=5
DYNAMIC_ROUTE_CENTER_FOUND_MIN=9
DYNAMIC_LATE_RESCUE_MAX_FOUND=14
OPPORTUNITY_REMAINING_RISK_FLOOR=0.12
TAIL_REMAINING_RISK_FLOOR=0.20
SECOND_RESCUE_MAX_FOUND=15
SECOND_RESCUE_RING_RADIUS_M=1600.0
SECOND_RESCUE_TRAVEL_PENALTY=0.00012
SECOND_RESCUE_RISK_FLOOR=0.20
SECOND_RESCUE_MIN_COVERAGE=0.34
ADAPTIVE_RESCUE_MIN_FOUND=12
ADAPTIVE_RESCUE_MAX_FOUND=14
ADAPTIVE_RESCUE_RISK_FLOOR=0.40
ADAPTIVE_RESCUE_BUDGET=3
SHADOW_RESCUE_RING_RADIUS_M=2100.0
SHADOW_RESCUE_BUDGET=6
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    for k in range(TAIL_BUDGET):
        ...
        clear_detected()
    # 首次后验探索仍未找齐时，用远端实验参数执行一次1600米定向外圈找补。
    value_rescue_moved=False
    second_rescue_risk=remaining_active_risk(beliefs,len(found)) if len(found)>=10 else 1.0
    if (len(found)<=SECOND_RESCUE_MAX_FOUND and second_rescue_risk>SECOND_RESCUE_RISK_FLOOR
            and time.monotonic()-start<deadline):
        unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
        if unknown:
            from q4_targeted_rescue_v3 import select_state_tail_probe
            q,p=select_state_tail_probe(beliefs[unknown[0]],client.current_position_m,mode='balanced',
                ring_radius=SECOND_RESCUE_RING_RADIUS_M,travel_penalty=SECOND_RESCUE_TRAVEL_PENALTY)
            if (q is not None and p>=SECOND_RESCUE_MIN_COVERAGE
                    and (not visited or min(np.linalg.norm(q-np.asarray(x)) for x in visited)>=MIN_SCAN_SEPARATION)):
                print(f'价值门控外圈救援 风险{second_rescue_risk:.3f} 覆盖{p:.3f}')
                value_rescue_moved=True
                scan(q,force=True);tails.append(q.tolist())
                clear_detected()
    # 低首点价值但高风险的12--13源状态，需要连续扫描后才会形成高价值外圈点。
    if not value_rescue_moved:
        for k in range(ADAPTIVE_RESCUE_BUDGET):
            if not (ADAPTIVE_RESCUE_MIN_FOUND<=len(found)<=ADAPTIVE_RESCUE_MAX_FOUND):break
            if time.monotonic()-start>=deadline:break
            unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
            if not unknown:break
            risk=remaining_active_risk(beliefs,len(found))
            if risk<ADAPTIVE_RESCUE_RISK_FLOOR:break
            from q4_targeted_rescue_v3 import select_state_tail_probe
            q,p=select_state_tail_probe(beliefs[unknown[0]],client.current_position_m,mode='balanced',
                ring_radius=SECOND_RESCUE_RING_RADIUS_M,travel_penalty=SECOND_RESCUE_TRAVEL_PENALTY)
            if q is None or (visited and min(np.linalg.norm(q-np.asarray(x)) for x in visited)<MIN_SCAN_SEPARATION):break
            print(f'高风险连续找补{k+1} 风险{risk:.3f} 覆盖{p:.3f}')
            scan(q,force=True);tails.append(q.tolist())
            clear_detected()
    # 内侧外圈反复无信号时，真实残余源可能位于同径向更外侧且波束朝外。
    # 仅在计数下界或内环失效证据强时启动，沿共享无信号点的径向阴影动态外推。
    shadow_unknown=[c for c,b in beliefs.items() if b['status']=='unknown']
    if shadow_unknown and len(found)<16:
        shadow_risk=remaining_active_risk(beliefs,len(found)) if len(found)>=10 else 1.0
        from q4_targeted_rescue_v3 import select_state_tail_probe,rank_radial_shadow_probes
        _q16,_p16=select_state_tail_probe(beliefs[shadow_unknown[0]],client.current_position_m,
            mode='balanced',ring_radius=1600.0,travel_penalty=SECOND_RESCUE_TRAVEL_PENALTY)
        _q21,_p21=select_state_tail_probe(beliefs[shadow_unknown[0]],client.current_position_m,
            mode='balanced',ring_radius=SHADOW_RESCUE_RING_RADIUS_M,travel_penalty=SECOND_RESCUE_TRAVEL_PENALTY)
        shadow_trigger=(len(found)<10
            or (len(found)==12 and shadow_risk>=0.30 and _p16<0.10)
            or (len(found)==14 and _p16<0.08)
            or (len(found)==14 and shadow_risk<0.22 and _p16<0.10 and _p21>0.28))
        if shadow_trigger:
            shadow_used=[]
            print(f'径向阴影找补启动 已发现{len(found)} 风险{shadow_risk:.3f} 内环{_p16:.3f} 外环{_p21:.3f}')
            for k in range(SHADOW_RESCUE_BUDGET):
                candidates=rank_radial_shadow_probes(beliefs,client.current_position_m,
                    ring_radius=SHADOW_RESCUE_RING_RADIUS_M)
                candidates=[item for item in candidates
                    if (not shadow_used or min(np.linalg.norm(item['point']-q) for q in shadow_used)>=300.0)
                    and (not visited or min(np.linalg.norm(item['point']-np.asarray(q)) for q in visited)>=120.0)]
                if not candidates or len(found)>=16 or time.monotonic()-start>=deadline:
                    break
                max_support=max(item['support'] for item in candidates)
                strong=[item for item in candidates if item['support']==max_support]
                item=min(strong,key=lambda x:float(np.linalg.norm(x['point']-client.current_position_m)))
                before=len(found); q=np.asarray(item['point'],float)
                print(f"径向阴影找补{k+1} 角度{item['angle_deg']:.1f} 支持{item['support']}")
                scan(q,force=True); tails.append(q.tolist()); shadow_used.append(q)
                clear_detected()
                if len(found)>before and len(found)>=10:
                    new_risk=remaining_active_risk(beliefs,len(found))
                    if new_risk<0.10:
                        break
    client.exit()
