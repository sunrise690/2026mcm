...
VERSION='v456-risk-gated-outer-rescue'
...
def run_q4(client, *, case_code, real_margin_s=45.0, scan_detected_again=True, fast=False, full_census=False):
    ...
    if (len(found)<=SECOND_RESCUE_MAX_FOUND and second_rescue_risk>SECOND_RESCUE_RISK_FLOOR
        ...
        if unknown:
            ...
            if q is not None and (not visited or min(np.linalg.norm(q-np.asarray(x)) for x in visited)>=MIN_SCAN_SEPARATION):
                print(f'风险门控外圈救援 风险{second_rescue_risk:.3f} 覆盖{p:.3f}')
                scan(q,force=True);tails.append(q.tolist())
                clear_detected()
    client.exit()
